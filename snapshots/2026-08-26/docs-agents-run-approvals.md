...
Open in VS Code Insiders
setting.
Assisted permissions is only available for agents that run on the Agent Host. To show this option in the permissions picker, enable
...
Your organization can configure fine-grained managed permissions for shell commands, file operations, and domains. Managed rules take precedence over the session permission level. A managed rule can still require approval or block an operation when you use Bypass Approvals or Autopilot.
Important
...
Open in VS Code
Open in VS Code Insiders
(experimental): when set to outsideWorkspace (default), require approval for terminal commands that write files outside your workspace. Writes to the OS temporary folder (/tmp on macOS and Linux, %TEMP% on Windows) are exempt when session-level command approval is active.
