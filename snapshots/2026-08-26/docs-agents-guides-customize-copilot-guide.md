...
In the Overview tab, enter a prompt that asks the agent to analyze your codebase and create workspace instructions. For example:
...
Answer any clarifying questions. The agent analyzes your repository and generates .github/copilot-instructions.md.
...
Ask the agent to complete the task.
Check that it places files in the correct directories and follows your documented patterns.
Check that it uses the preferred libraries and runs the correct validation commands.
Expand the References section in the chat response and confirm that copilot-instructions.md was included.
...
In the Overview tab, describe the targeted workspace instructions you want to create. Base the request on a language, framework, directory, or task in your repository. For example:
...
Review the generated file and its applyTo frontmatter value. The glob pattern determines which files receive the instructions automatically.
Open a matching file and ask the agent to make a small change. Check the response's References section to confirm that the targeted instructions were included.
...
Open the Agent Customizations editor.
In the Overview tab, describe a recurring workflow from your repository and ask the agent to create a workspace skill.
Review the generated SKILL.md. Make sure its description clearly states what the skill does and when the agent should load it.
Review any scripts, templates, or other resources in the skill directory.
Ask the agent to perform a matching task and check that it loads the skill and follows the documented workflow.
...
Open the Agent Customizations editor.
In the Overview tab, describe the role, its responsibilities, and the tools it should use. For example:
...
Review the generated agent file. Check that its instructions describe the role and that its tool configuration matches the actions it should perform.
Select the custom agent from the agents dropdown and ask it to research how a feature or subsystem works in your repository.
Verify that it identifies the relevant files, traces the implementation, supports its findings with evidence, and stays within its configured tool access.
...
Next steps
Create and manage customizations from the Agent Customizations editor.
Add MCP servers to connect the agent to external tools and services.
Set up hooks when an action must run at a specific agent lifecycle event.
8/19/2026
Support
Privacy
Manage Cookies
