...
context
No
(Experimental) Controls how the skill is loaded. Defaults to inline (the skill's instructions are added to the parent agent's context). Set to fork to run the skill in a dedicated subagent context. See Run a skill in a forked context.
Important
