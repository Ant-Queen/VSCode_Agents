...
Open in VS Code
Open in VS Code Insiders
to use BYOK models in those sessions. This setting is experimental and takes effect only after the agent host process is restarted.
...
Open in VS Code
Open in VS Code Insiders
: Override the model used for fast, lightweight utility flows, such as commit messages, rename suggestions, branch name generation, prompt categorization, and intent detection. A fast and inexpensive model is recommended for this setting.
...
Open in VS Code
Open in VS Code Insiders
to a BYOK model to enable utility features like title generation and commit message creation.
