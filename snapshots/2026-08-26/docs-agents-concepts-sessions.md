...
Hand off a session
Handoff changes the session target from one harness to another and carries the full conversation history and context with it. Use handoff when a different harness or execution environment is a better fit for the next part of the task.
...
Continue in the cloud: hand off a well-scoped task to a cloud harness for remote execution and a pull request workflow.
