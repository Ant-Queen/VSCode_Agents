...
Review before applying. All file changes surface in a diff view for keep/undo decisions, and checkpoints let you roll back a session.
...
Network domain: controls whether the agent can fetch content from a URL. VS Code prompts you to trust a domain before making requests to it, integrated with the Trusted Domains list. You can also enable
chat.agent.networkFilter
Open in VS Code
Open in VS Code Insiders
to restrict which domains agent tools and sandboxed terminal commands can access.
...
VS Code applies sandboxing to terminal commands (runInTerminal agent tool) that are executed during an agent session, including Copilot agent-host sessions that use the VS Code agent terminal integration. Learn how to configure agent sandboxing.
...
Sandbox enablement and unrestricted network access are separate controls. When sandboxing is enabled and
chat.agent.sandbox.allowNetwork
Open in VS Code
Open in VS Code Insiders
is off, all outbound network access is blocked unless you explicitly allow specific domains. When
chat.agent.sandbox.allowNetwork
Open in VS Code
Open in VS Code Insiders
is on, commands can reach external services freely while file system restrictions still apply. On macOS and Linux,
chat.agent.sandbox.enabled
Open in VS Code
Open in VS Code Insiders
controls sandbox enablement and accepts off (default) or on.
VS Code provides network domain filtering that applies to both agent tools (fetch tool, integrated browser) and sandboxed terminal commands. Enable
chat.agent.networkFilter
Open in VS Code
Open in VS Code Insiders
to activate network filtering. Use
...
Open in VS Code
Open in VS Code Insiders
to control which domains the agent can access. Learn how to configure network access.
...
Prerequisites
macOS
Apple's sandboxing framework ("Seatbelt"), built into the operating system. Enforces fine-grained file system and network restrictions at the kernel level.
None. Works out of the box.
Linux and WSL2
bubblewrap for file system isolation and socat for network proxying.
...
Open in VS Code
Open in VS Code Insiders
setting provides network domain filtering for agent tools like the fetch tool and integrated browser, independently of sandboxing. When both sandboxing and network filtering are enabled, network rules apply to all agent tools and terminal commands.
