...
Core AI features: general, code editing, chat, agents, agent sessions, and inline chat.
Agent customization: custom instructions, prompt files, custom agents, agent skills, and agent plugins.
Other AI features: code review, source control, memory, observability, debugging, testing, notebooks, voice and dictation, and accessibility.
General settings
...
Open in VS Code
Open in VS Code Insiders
Override the language model used for fast, lightweight utility flows, such as commit messages, pull request titles and descriptions, rename suggestions, and intent detection. A fast, inexpensive model is recommended.
"Default"
...
Show Assisted permissions in supported Agent Host permission pickers. An LLM judge evaluates the risk of each tool call and asks for your approval when it does not approve a call.
true (Insiders)
false (Stable)
...
Wire up the BYOK language model bridge so extension-provided BYOK models can run in Agent Host sessions. Changes are synchronized to the running Agent Host.
false
...
Open in VS Code Insiders
(Experimental)
Enable Agent Merge and its commands. Agent Merge monitors a session's pull request and can ask the agent to address selected blockers.
false
chat.agentMerge.addressReviews
Open in VS Code
Open in VS Code Insiders
(Experimental)
Address unresolved review threads, changes-requested reviews, and new pull request comments from repository maintainers or the Copilot pull request reviewer.
true
chat.agentMerge.fixCI
Open in VS Code
Open in VS Code Insiders
(Experimental)
Ask the agent to fix failed required CI checks.
true
chat.agentMerge.resolveConflicts
Open in VS Code
Open in VS Code Insiders
(Experimental)
Ask the agent to update branches that are behind and resolve merge conflicts.
true
chat.agentMerge.mergePullRequest
Open in VS Code
Open in VS Code Insiders
(Experimental)
Control whether Agent Merge merges or enqueues the pull request when it is ready. always merges after any selected maintenance work, ifUnchanged merges only until an agent repair turn changes the pull request, and never turns off automatic merging.
"never"
chat.agentMerge.mergeMethod
Open in VS Code
Open in VS Code Insiders
(Experimental)
Select the merge method. auto uses the first repository-compatible method in this order: squash, merge commit, or rebase. You can also require squash, merge, or rebase.
"auto"
chat.agentMerge.replyAttribution
Open in VS Code
Open in VS Code Insiders
(Experimental)
Include an automated-reply attribution in review-thread replies posted by Agent Merge.
true
Inline chat settings
Setting and Description
Default
...
Open in VS Code
Open in VS Code Insiders
Enable file logging for agent debug logs in extension host chat sessions. This setting writes debug events to disk for the Agent Debug Logs panel and the /troubleshoot slash command. Reload the window after changing this setting.
false
chat.agentHost.agentDebugLog.enabled
Open in VS Code
Open in VS Code Insiders
(Experimental)
Enable agent debug logging for Agent Host sessions and show their debug events in the Agent Debug Logs panel. Only sessions that run while this setting is enabled are captured.
false
...
Open in VS Code
Open in VS Code Insiders
Built-in dictation is experimental and might change or be removed.
...
Open in VS Code Insiders
Built-in dictation is experimental and might change or be removed.
Provides a language hint for dictation. Use auto to use the system language.
"auto"
Accessibility settings
Setting and Description
Default
